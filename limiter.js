!function($){
    ...
    Limiter.prototype = {
        render : function() {
            ...
            this.options.tpl = self.options.defaultTpl;
            this.count();
            this.$target.on('keyup blur', function(ev){
                self.count();
            });
        },
        ...
        _tr2Simple:function(trStr){
            ...
            for(var i = 0; i < trStr.length; i++){
                var isTr = false;
                for(var j = 0; j < TRADITIONAL_CHAR.length; j++){
                    if(TRADITIONAL_CHAR.charAt(j) == trStr.charAt(i)){
                        rs.push(SIMPLIFIED_CHAR.charAt(j));
                        isTr = true;
                        break;
                    }
                }
    ...
    $.fn.limiter.defaults = {
        ...
        // default template "<span class="ks-letter-count">您还可以输入<em class="J_LetterRemain">{remain}</em>个字</span>"

        defaultTpl : '<span class="ks-letter-count"><em class="J_LetterRemain">{remain}</em>字节</span>',

        // exceed template "<span class="ks-letter-count">已经超出<em class="J_LetterRemain exceed-letter">{remain}</em>个字</span>"
